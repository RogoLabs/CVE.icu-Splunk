...
class GitHubClient:
    ...
    def _configure_session(self) -> None:
        ...
        if self.github_token:
            ...
            self.logger.info(
                "No GitHub token - using unauthenticated access (60 req/hr)"
            )
        ...
        **kwargs,
        ...
        try:
            response = self._session.request(
                method=method, url=url, stream=stream, timeout=timeout or 30, **kwargs
            )
    ...
    def get_releases(
        self, per_page: int = 100, max_releases: Optional[int] = None
    ...
    def find_delta_releases_since(
        self, since_tag: str, max_releases: int = 100
    ) -> List[Dict[str, Any]]:
        """
    ...
    def download_release_asset(
        self, asset_url: str, destination: Optional[str] = None
    ) -> str:
        """
        ...
        with open(destination, "wb") as f:
            for chunk in response.iter_content(chunk_size=self.CHUNK_SIZE):
                if chunk:
                    ...
                    if (
                        total_size > 0
                        and downloaded % (10 * 1024 * 1024) < self.CHUNK_SIZE
                    ):
                        progress = (downloaded / total_size) * 100
                        self.logger.debug(f"Download progress: {progress:.1f}%")

        self.logger.info(
            f"Downloaded {downloaded / (1024 * 1024):.1f}MB to {destination}"
        )
        return destination

    def stream_zip_contents(
        self, zip_path: str, file_pattern: str = ".json"
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            for file_info in zf.infolist():
                ...
                if file_info.filename.endswith(".zip"):
                    ...
                    with zf.open(file_info) as nested_zip_data:
                        ...
                        try:
                            ...
                            yield from self.stream_zip_contents(
                                nested_path, file_pattern
                            )
                        finally:
                            # Clean up temp file
                            try:
                ...
                if (
                    "delta" in file_info.filename
                    and "manifest" in file_info.filename.lower()
                ):
                    continue
                ...
                try:
                    ...
                    self.logger.warning(f"Malformed JSON in {file_info.filename}: {e}")
                except Exception as e:
                    self.logger.error(f"Error reading {file_info.filename}: {e}")

    def get_asset_for_release(
        self, release: Dict[str, Any], name_contains: str
    ) -> Optional[Dict[str, Any]]:
        """
